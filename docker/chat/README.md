...
- `POST /api/live/session` → optional body `{"sessionId?":"..."}` → `{ "websocketUrl", "handshake", "model", "apiVersion" }` for Gemini Live voice. **`websocketUrl`** targets **`/api/live/relay/{id}`** on this service (server relays to Google with `Authorization: Token …`). Send **`handshake`** as the first WebSocket message after connect. Requires **`GEMINI_API_KEY`** and a loaded knowledge pack. Local dev: proxy **`/api/chat`**, **`/api/live/session`**, and **`/api/live/relay/`** (WebSocket) to uvicorn.
