...
if (!contactUrl) {
  console.error(
    'usage: sync-site-api-urls.mjs <contactApiUrl> [chatApiUrl]\n' +
      ...
      '  chatApiUrl: optional full URL ending in /api/chat. Voice (browser mic) needs a WebSocket-capable\n' +
      '              chat host (ECS/ALB) so the FE relay path works; Lambda-only execute-api will fail voice with 1011.'
  )
  process.exit(1)
}
